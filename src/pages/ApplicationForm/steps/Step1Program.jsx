...
export default function Step1Program({ data, update }) {
  ...
  // Auto-select role from URL on component mount
  useEffect(() => {
    if (roleFromURL) {
      // Check if the role from URL is valid
      const validRole = APPLICATION_ROLE_OPTIONS.find(r => r.value === roleFromURL);
      if (validRole) {
        update("step1.applicationRole", roleFromURL);
        // If role is CM, clear any program selection
        if (roleFromURL === "cm" && s1.program) {
          update("step1.program", "");
        }
      }
    }
  }, [roleFromURL]);
  ...
  const set = (field) => (e) =>
    update(`step1.${field}`, e?.target ? e.target.value : e);
  const setAddr = (field) => (e) =>
    update(`step1.currentMailingAddress.${field}`, e.target.value);

  // Get the selected role label
  const selectedRoleLabel = APPLICATION_ROLE_OPTIONS.find(r => r.value === s1.applicationRole)?.label;
    ...
    <div className="space-y-6">
      ...
      <Card icon={<CalendarIcon />} title="Entering Session">
        ...
        <Field label="Choose your Application Role" required>
          <div className="flex flex-wrap gap-3 mt-2">
            {roleFromURL ? (
              // If role is in URL, show only that role button
              APPLICATION_ROLE_OPTIONS.filter(r => r.value === roleFromURL).map((r) => (
                <button
                  key={r.value}
                  type="button"
                  className={`px-4 py-2.5 rounded-2xl border text-sm font-medium transition bg-[#0F4C81] text-white border-[#0F4C81] cursor-default`}
                  disabled
                >
                  {r.label}
                </button>
              ...
              // If no role in URL, show all options
              APPLICATION_ROLE_OPTIONS.map((r) => (
                <button
                  ...
                  onClick={() => {
                    ...
                    if (r.value === "cm" && s1.program)
                      update("step1.program", "");
                  }}
                  className={`px-4 py-2.5 rounded-2xl border text-sm font-medium transition ${
                    s1.applicationRole === r.value
                      ? "bg-[#0F4C81] text-white border-[#0F4C81]"
                      : "bg-[#F5F8FC] text-[#0D1B2E] border-[#0F4C81]/15"
                  }`}
                >
                  {r.label}
                </button>
          ...
          {roleFromURL && (
            <p className="text-xs text-[#0F4C81] mt-2">
              Selected from application: {selectedRoleLabel}
            </p>
          )}
        </Field>
            ...
            <div className="grid sm:grid-cols-2 gap-y-3 gap-x-8 mt-2">
              {PROGRAM_OPTIONS.map((p) => (
                <RadioCircle
                  key={p}
                  label={p}
                  selected={s1.program === p}
                  onSelect={() => update("step1.program", p)}
                />
              ))}
            </div>
          </Field>
      ...
      <Card icon={<IdIcon />} title="Applicant Information">
        ...
        <div className="grid sm:grid-cols-2 gap-x-6 gap-y-5">
          ...
          <Field label="Last Name (as in passport)" required>
            <TextInput
              required
          ...
          <div className="grid sm:grid-cols-2 gap-x-6 gap-y-5 sm:col-span-2">
            ...
            <Field
              label="Aadhar Card / PAN Number"
              hint="(for Indian Applicants)"
            >
              <TextInput
                pattern="[A-Za-z0-9]{6,12}"
              ...
              if (Math.abs(dobAge - ageNum) > 1) {
                return (
                  <p className="text-xs text-red-600 font-medium mt-1.5 flex items-center gap-1">
                    ...
                    Age ({ageNum}) doesn't match Date of Birth — calculated age
                    is {dobAge}
                  </p>
                );
              }
          ...
          <Field label="Current Mailing Address (Number and Street)" required>
            <TextInput
              ...
              value={s1.currentMailingAddress.street}
          ...
          <Field label="State/Province" required>
            <TextInput
              ...
              value={s1.currentMailingAddress.state}
          ...
          <Field label="Country" required>
            <TextInput
              ...
              value={s1.currentMailingAddress.country}
          ...
          <Field label="State & Country of Birth" required>
            <TextInput
              required
              placeholder="Enter State & Country of birth"
              value={s1.cityStateCountryOfBirth}
              onChange={set("cityStateCountryOfBirth")}
            />
          </Field>
        </div>
      </Card>
      ...
      <Card
        icon={<ShieldIcon />}
        title="Citizenship Status for International Applicants"
      >
        {(() => {
          const isUK =
            s1.primaryCountryOfCitizenship?.trim().toLowerCase() ===
            "united kingdom";
          return (
            <>
                ...
                <div className="flex flex-col gap-2">
                  <RadioCircle
                    ...
                    onSelect={() => {
                      ...
                      update(
                        "step1.primaryCountryOfCitizenship",
                        "United Kingdom",
                      );
                    }}
                  />
                ...
                <>
                    ...
                    {s1.dualCitizenship === "Yes" && (
                      <div className="mt-3">
                        <MultiCountrySelect
                          ...
                          onChange={(val) =>
                            update("step1.dualCitizenshipCountries", val)
                          }
                        />
                      </div>
                    )}
                  ...
                  <Field label="Current Country of Residence" required>
                    <CountrySelect
                      ...
                      onChange={(e) =>
                        update(
                          "step1.currentCountryOfResidence",
                          e.target.value,
                        )
                      }
                      required
                  ...
                  <div className="mt-5">
                    <p className="text-sm font-semibold text-[#0D1B2E] mb-2">
                      Have you previously travelled to or stayed in the United
                      Kingdom?
                      <span className="text-red-500"> *</span>
                    </p>
                    <div className="flex flex-col gap-2">
                    ...
                    {s1.previouslyInUK === "Yes" && (
                      <div className="mt-3 pl-4 border-l-2 border-[#0F4C81]/20 flex flex-col gap-3">
                        <Field label="Visa Type(s) Held" required>
                          <TextInput
                            placeholder="Enter visa type(s) held"
                            value={s1.ukVisaTypesHeld}
                            onChange={set("ukVisaTypesHeld")}
                          />
                        </Field>
                        <Field label="Duration of Stay" required>
                          <TextInput
                            placeholder="e.g. 3 months, 1 year"
                            value={s1.ukDurationOfStay}
                            onChange={set("ukDurationOfStay")}
                          />
                        </Field>
                      </div>
                    )}
                  ...
                  <div className="mt-5">
                    <p className="text-sm font-semibold text-[#0D1B2E] mb-2">
                      Have you ever{" "}
                      <span className="underline">been refused</span> a UK visa
                      or entry into the UK?
                      <span className="text-red-500"> *</span>
                    </p>
                    <div className="flex flex-col gap-2">
                      {["No", "Yes"].map((opt) => (
                    ...
                    {s1.refusedUKVisa === "Yes" && (
                      <TextInput
                        required
                        className="mt-3"
                        placeholder="Provide details of refusal"
                        value={s1.refusedUKVisaDetails}
                        onChange={set("refusedUKVisaDetails")}
                      />
                    )}
                  </div>
                  ...
                  <div className="flex flex-col gap-3 pl-1">
                    <Field label="Date of Most Recent UK Entry">
                      <DateInput
                        value={s1.ukRecentEntryDate}
                        onChange={set("ukRecentEntryDate")}
                      />
                    </Field>
                    <Field label="Current UK Visa / Immigration Status">
                      <TextInput
                        placeholder="e.g. Student Visa, Skilled Worker"
                        value={s1.ukCurrentVisaImmigrationStatus}
                        onChange={set("ukCurrentVisaImmigrationStatus")}
                      />
                    </Field>
                    <Field label="Visa Expiry Date">
                      <DateInput
                        value={s1.ukVisaExpiryDate}
                        onChange={set("ukVisaExpiryDate")}
                      />
                    </Field>
                  </div>
                </>
